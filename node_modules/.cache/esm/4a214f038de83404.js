let express,Router,engine,sqlite3,open,cookieParser;_8de‍.w("express",[["default",["express"],function(v){express=v}],["Router",["Router"],function(v){Router=v}]]);_8de‍.w("express-handlebars",[["engine",["engine"],function(v){engine=v}]]);_8de‍.w("sqlite3",[["default",["sqlite3"],function(v){sqlite3=v}]]);_8de‍.w("sqlite",[["open",["open"],function(v){open=v}]]);_8de‍.w("cookie-parser",[["default",["cookieParser"],function(v){cookieParser=v}]]);
...
app.post('/course',cookieVal, async (req,res) => {
    ...
    if(priv.priv == 'teacher'){
        var Grades = await db.all("SELECT *,((grade1 + grade2 + grade3)/3) AS avg FROM Users INNER JOIN Grades INNER JOIN Courses WHERE Users.u_id = Grades.u_id AND Courses.c_id = Grades.c_id AND name = ? AND teacherName = ?;",Object.values(courseID).toString(),req.cookies.user)
        res.render('logTeach', {
            Courses,
            Grades,
        ...
        var Grades = await db.all("SELECT *,((grade1 + grade2 + grade3)/3) AS avg FROM Users INNER JOIN Grades INNER JOIN Courses WHERE Users.u_id = Grades.u_id AND Courses.c_id = Grades.c_id AND name = ? AND username = ?;",Object.values(courseID).toString(),req.cookies.user)
        res.render('log', {
            Courses,
            Grades,
            PageTitle: Object.values(course).toString() + ' - Management++',
